...
#ifndef LatCore_XmlReader_hpp_
#define LatCore_XmlReader_hpp_

#include <LatCore/LatCore.hpp>

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wsign-conversion"
#include <LatCore/XML/tinyxml2.hpp>
                                        ...
                                        Strs... nodeNames)
{
    ...
    for (unsigned int i = 0; i < nName; ++i)
    {
        ...
        if (!node)
        {
            XML_ERROR("XML node " + name[i] + " not found");
        }
    }
...
T XmlReader::getValue(const XmlNode *node)
{
    if (node)
    {
        if (node->GetText())
        {
            return LatCore::strTo<T>(node->GetText());
        }
        else
        {
            return T();
